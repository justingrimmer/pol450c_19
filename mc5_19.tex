...
\boldsymbol{\beta}^{*} & \longrightarrow^{D} & \text{Multivariate Normal}\left(\boldsymbol{\beta}, I_{N}(\boldsymbol{\beta}^{*})^{-1}\right) \nonumber 
\end{eqnarray}}\pause 
...
h(\boldsymbol{\beta})^{'}
\underbrace{(\boldsymbol{\beta}^{*} - \boldsymbol{\beta})}_{\text{MVN}(0, I_{N}(\boldsymbol{\beta})^{-1}} \right)  \nonumber 
\end{eqnarray}
\pause 
...
\begin{eqnarray}
\invisible<1-3>{\boldsymbol{\beta}^{m} & \sim & \text{MVN}\left(\boldsymbol{\beta}^{*}, I_{N}(\boldsymbol{\beta})^{-1} \right) \nonumber\\} \pause 
