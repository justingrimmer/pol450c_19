...
\begin{eqnarray}
\invisible<1-2>{\sqrt{n}(\theta^{*} - \theta_{0}) & \rightarrow^{d} & \text{Normal}(0, \frac{1}{I_{n}(\theta_{0})} ) \nonumber } \pause \\
\invisible<1-3>{\sqrt{n}(\theta^{*} - \theta_{0}) & \rightarrow^{d} & \text{Normal}(\theta_{0}, \frac{1}{I_{n} (\theta^{*})})} \nonumber
\end{eqnarray}
